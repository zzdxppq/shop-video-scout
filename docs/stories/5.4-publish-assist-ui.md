...
**Story Deliverables**:
- 发布辅助面板组件 (PublishAssistPanel)
- 话题标签芯片组件 (TopicChips)
- 标题列表组件 (TitleList)
- 剪贴板复制逻辑 (useClipboard composable)
- 发布辅助数据获取 (usePublishAssist composable)
- API模块 (publish.ts)
...
| ID | Rule |
|----|------|
| BR-1.1 | 话题标签显示为芯片/标签样式（圆角矩形，带背景色） |
| BR-1.2 | 复制后显示成功提示Toast（"已复制到剪贴板"，2秒消失） |
| BR-1.3 | 一键复制时所有话题用空格分隔（如 "#话题1 #话题2 #话题3"） |
| BR-1.4 | 单个话题点击时复制该话题文本 |
| BR-1.5 | 话题按API返回顺序显示（推荐优先级排序） |
| BR-1.6 | 话题芯片最多显示10个（API返回5-10个） |

**Data Validation**
| Field | Type | Required | Rules | Error Message |
|-------|------|----------|-------|---------------|
| topics | string[] | Y | length 5-10 | 话题数据加载失败 |
| topic | string | Y | starts with # | 话题格式异常 |

**Error Handling**
| Scenario | Code | Message | Action |
|----------|------|---------|--------|
| API加载失败 | 500 | 话题加载失败，请刷新重试 | 显示重试按钮 |
| 剪贴板复制失败 | - | 复制失败，请手动复制 | 显示失败Toast |
| 空数据 | - | 暂无话题推荐 | 显示空状态 |

**UI Interaction**
| Trigger | Behavior |
|---------|----------|
| 点击一键复制按钮 | 复制所有话题到剪贴板，显示成功Toast |
| 点击单个话题芯片 | 复制该话题，显示"已复制: #话题"Toast |
| 悬停话题芯片 | 显示hover状态（颜色加深） |
...
| ID | Rule |
|----|------|
| BR-2.1 | 标题显示为卡片列表样式（每个标题一行） |
| BR-2.2 | 第一个标题标记为"推荐"（带推荐徽章） |
| BR-2.3 | 每个标题右侧有复制图标按钮 |
| BR-2.4 | 复制后显示成功Toast |
| BR-2.5 | 标题最多显示5个（API返回3-5个） |
| BR-2.6 | 标题超长时截断显示，悬停显示完整内容 |

**Data Validation**
| Field | Type | Required | Rules | Error Message |
|-------|------|----------|-------|---------------|
| titles | string[] | Y | length 3-5 | 标题数据加载失败 |
| title | string | Y | 20-50 chars | 标题格式异常 |

**Error Handling**
| Scenario | Code | Message | Action |
|----------|------|---------|--------|
| API加载失败 | 500 | 标题加载失败，请刷新重试 | 显示重试按钮 |
| 剪贴板复制失败 | - | 复制失败，请手动复制 | 显示失败Toast |
| 空数据 | - | 暂无标题推荐 | 显示空状态 |
...
| Trigger | Behavior |
|---------|----------|
| 点击复制按钮 | 复制对应标题，显示成功Toast |
| 悬停标题 | 长标题显示完整内容tooltip |
| 悬停复制按钮 | 显示"点击复制"提示 |
...
| ID | Rule |
|----|------|
| BR-3.1 | 显示重新生成按钮及剩余次数（"重新生成 (剩余2次)"） |
| BR-3.2 | 点击后按钮显示loading状态，禁用点击 |
| BR-3.3 | 剩余次数为0时按钮禁用，显示"已达上限" |
| BR-3.4 | 生成成功后更新内容，显示成功Toast |
| BR-3.5 | 生成失败后显示错误Toast，保留原内容 |
...
| Scenario | Code | Message | Action |
|----------|------|---------|--------|
| 超过次数限制 | 1031 | 已达到重新生成次数上限 | 禁用按钮 |
| API失败 | 500 | 重新生成失败，请重试 | 恢复按钮状态 |

**UI Interaction**
| Trigger | Behavior |
|---------|----------|
| 点击重新生成 | 显示loading，调用API |
| API成功 | 更新内容，显示成功Toast |
| API失败 | 显示错误Toast，恢复按钮 |
...
- [x] **T1: TopicChips组件实现** `[AC1]`
  ...
  - **Subtasks**:
    ...
    - [x] T1.4: Style chips with Tailwind:
      ...
      - Padding: px-3 py-1
      - Cursor: pointer
    - [x] T1.5: Handle empty state (show "暂无话题推荐")
  - **Complexity**: Low
  - **Error Handling**:
    | Scenario | Handler |
    |----------|---------|
  ...
  **Test Specs** (unit):
  ...
  |----------|-------|----------|-------|
  | Render topics as chips | ["#话题1", "#话题2"] | 2 chip elements | unit |
  | Click chip copies | click chip "#话题1" | copyToClipboard("#话题1") called | unit |
  | Click copyAll | click "一键复制" | copyToClipboard("#话题1 #话题2") called | unit |
  | Empty state | [] | shows "暂无话题推荐" | unit |
...
- [x] **T2: TitleList组件实现** `[AC2]`
  ...
  - **Subtasks**:
    - [x] T2.1: Create TitleCard.vue component:
      ...
      - Emits: `copy`
      - Display: title text, copy icon button, optional "推荐" badge
  ...
  **Test Specs** (unit):
  | Scenario | Input | Expected | Level |
  |----------|-------|----------|-------|
  | Render all titles | ["标题1", "标题2", "标题3"] | 3 TitleCard components | unit |
  ...
  | Recommended badge shown | isRecommended=true | shows "推荐" badge | unit |
  | Empty state | [] | shows "暂无标题推荐" | unit |
...
- [x] **T3: PublishAssistPanel组件实现** `[AC1, AC2, AC3]`
  - **File Locations**:
  ...
  **Test Specs** (unit):
  ...
  | Regenerate button shows remaining | remaining=2 | "(剩余2次)" text | unit |
  | Regenerate button disabled | remaining=0 | button disabled, "已达上限" | unit |
...
- [x] **T4: 集成到VideoPreviewView** `[ALL ACs]`
  - **File Locations**:
...
├── VideoPlayer.vue
├── ShotList.vue
├── PublishAssistPanel.vue (NEW)
│   ├── Section: 话题标签
│   │   ├── TopicChips.vue (NEW)
│   │   │   └── Chip elements (click to copy)
│   │   └── "一键复制" button
│   ├── Section: 推荐标题
│   │   └── TitleList.vue (NEW)
│   │       └── TitleCard.vue (NEW, multiple)
│   │           ├── Title text
│   │           ├── "推荐" badge (first only)
│   │           └── Copy button
│   └── RegenerateButton.vue (NEW)
...
**Binding Status**: ✅ Verified by Dev

### Testing Requirements

- **单元测试重点** (Vitest + Vue Test Utils):
...
|----|---------------|---------------|------|--------|
| AC1 | TopicChips.vue, useClipboard.ts | TopicChips.test.ts | unit | ✅ |
| AC2 | TitleList.vue, TitleCard.vue | TitleList.test.ts | unit | ✅ |
| AC3 | PublishAssistPanel.vue, RegenerateButton.vue, usePublishAssist.ts | PublishAssistPanel.test.ts | unit | ✅ |
...
|------------|-----------|--------|
| Directory structure | §12.1 Code Organization | ✅ api/, composables/, components/ |
| API path | Story 5.3 backend | ✅ /api/v1/publish/tasks/{id}/assist |
| Component naming | Vue 3 conventions | ✅ PascalCase |
| Composables naming | Vue 3 conventions | ✅ use* prefix |
| Integration point | VideoPreviewView.vue | ✅ Exists from Story 5.1 |
| Testing strategy | §12.4 | ✅ Vitest + Vue Test Utils |
...
| 2026-02-05 | SM | Draft → AwaitingArchReview | Full elaboration: TopicChips, TitleList, PublishAssistPanel components, useClipboard/usePublishAssist composables, API module, integration with VideoPreviewView. Corrected API path to /api/v1/publish/tasks/{id}/assist per Story 5.3. |
| 2026-02-05 16:00 | Architect | AwaitingArchReview → Approved | Score: 10.0/10, No issues. Simple tier - ready for Dev implementation |
| 2026-02-06 | Dev | Approved → Review | Implementation complete: TopicChips, TitleList, TitleCard, RegenerateButton, PublishAssistPanel components; useClipboard, usePublishAssist composables; API module; 44 unit tests. See dev log 5.4-dev-log.md |
