---
id: "5.2"
title: "下载导出功能"
epic: "5 - 预览导出与发布辅助"
status: Review
mode: plan
repository: monolith
priority: P0
estimated_complexity: low
tier: standard
---

# Story 5.2: 下载导出功能

## Story

**As a** 用户,
**I want** 下载合成完成的视频和素材,
**so that** 可以在本地保存和使用

### 🔌 APIs Provided by This Story

- `GET /api/v1/tasks/{id}/output/download`
- `GET /api/v1/tasks/{id}/assets-pack`

### 🔌 APIs Consumed by This Story

- `GET /api/v1/tasks/{id}/output` (from Story 4.3 - get output info)
- `GET /api/v1/tasks/{id}` (from Story 1.4 - get task details)

## Epic Context

**Epic**: 5 - 预览导出与发布辅助

实现成品视频的在线预览播放、下载导出功能，集成AI生成热门话题标签和视频标题推荐，提升发布效率。

**Story Deliverables**:
- 下载签名URL生成接口 (DownloadController)
- 素材包打包服务 (AssetPackService)
- 下载按钮组件 (DownloadButtons)
- 下载进度追踪 (useDownload composable)

## Acceptance Criteria

### AC1: 下载成品视频

**Scenario**
```gherkin
GIVEN 用户在结果页面
WHEN 用户点击下载成品
THEN
  - 获取带签名的下载URL
  - 触发浏览器下载
  - 显示下载进度（如支持）
```

**Business Rules**
| ID | Rule |
|----|------|
| BR-1.1 | 下载链接有效期1小时（3600秒签名） |
| BR-1.2 | 文件名格式：{店铺名}-探店视频-{日期}.mp4 |
| BR-1.3 | 仅任务状态为done时可下载 |
| BR-1.4 | 文件名特殊字符（/、\、:、*）替换为- |
| BR-1.5 | 日期格式：YYYYMMDD |
| BR-1.6 | 使用Content-Disposition: attachment触发下载 |

**Data Validation**
| Field | Type | Required | Rules | Error Message |
|-------|------|----------|-------|---------------|
| task_id | Long | Y | must exist | 任务不存在 |
| task.status | string | Y | must be 'done' | 视频尚未完成合成 |
| task.output_oss_key | string | Y | not empty | 视频文件不存在 |

**Error Handling**
| Scenario | Code | Message | Action |
|----------|------|---------|--------|
| 任务不存在 | 40303 | 任务不存在 | 返回错误 |
| 任务未完成 | 1025 | 视频尚未完成合成 | 返回错误，前端显示状态 |
| 链接过期 | 401 | 下载链接已过期 | 前端自动获取新链接 |
| OSS签名失败 | 500 | 生成下载链接失败 | 返回错误，显示重试 |

**UI Interaction**
| Trigger | Behavior |
|---------|----------|
| 点击"下载视频"按钮 | 显示loading状态，获取签名URL，触发下载 |
| 下载开始 | 按钮变为"下载中..."状态（如支持进度） |
| 下载完成 | Toast提示"下载完成"，按钮恢复 |
| 下载失败 | Toast提示错误，按钮恢复可点击 |

---

### AC2: 下载素材包

**Scenario**
```gherkin
GIVEN 用户需要原始推荐镜头
WHEN 用户点击下载素材包
THEN
  - 打包所有推荐镜头原片
  - 生成ZIP文件
  - 提供下载链接
```

**Business Rules**
| ID | Rule |
|----|------|
| BR-2.1 | 素材包仅包含推荐镜头（is_recommended=true的视频） |
| BR-2.2 | ZIP文件名：{店铺名}-素材包-{日期}.zip |
| BR-2.3 | ZIP内文件命名：{序号}-{分类}.mp4（如：01-美食.mp4） |
| BR-2.4 | 打包超时时间60秒，超时返回错误 |
| BR-2.5 | 单次打包最大10个视频文件 |
| BR-2.6 | ZIP文件缓存1小时，重复请求复用 |
| BR-2.7 | 缓存key：assets-pack:{task_id}:{videos_hash} |

**Data Validation**
| Field | Type | Required | Rules | Error Message |
|-------|------|----------|-------|---------------|
| task_id | Long | Y | must exist | 任务不存在 |
| task.status | string | Y | must be 'done' | 视频尚未完成合成 |
| videos | array | Y | has recommended videos | 无推荐镜头可下载 |

**Error Handling**
| Scenario | Code | Message | Action |
|----------|------|---------|--------|
| 无推荐镜头 | 40301 | 无推荐镜头可下载 | 返回错误，前端显示提示 |
| 打包失败 | 500 | 素材包生成失败，请重试 | 返回错误，显示重试按钮 |
| 打包超时 | 504 | 打包超时，请稍后重试 | 返回错误，显示重试 |
| 单个视频下载失败 | 500 | 部分素材打包失败 | 跳过失败文件，继续打包 |

**UI Interaction**
| Trigger | Behavior |
|---------|----------|
| 点击"下载素材包"按钮 | 显示"打包中..."状态，请求API |
| 打包完成 | 触发浏览器下载ZIP文件 |
| 打包失败 | Toast提示错误，显示重试按钮 |
| 无推荐镜头 | 按钮置灰，显示tooltip"无推荐镜头" |

---

## Tasks / Subtasks

### Infrastructure Tasks (T0)

- [ ] **T0: 下载模块基础架构** `[ALL ACs]`
  - **File Locations**:
    - `backend/task-service/src/main/java/com/shopvideoscout/task/controller/DownloadController.java` (create)
    - `backend/task-service/src/main/java/com/shopvideoscout/task/service/DownloadService.java` (create)
    - `backend/task-service/src/main/java/com/shopvideoscout/task/dto/DownloadUrlResponse.java` (create)
    - `frontend/src/api/download.ts` (create)
    - `frontend/src/composables/useDownload.ts` (create)
  - **Subtasks**:
    - [ ] T0.1: Create DownloadController.java with GET /tasks/{id}/output/download endpoint skeleton
    - [ ] T0.2: Create DownloadService.java interface with generateSignedUrl(taskId) method signature
    - [ ] T0.3: Create DownloadUrlResponse.java DTO (download_url, filename, expires_at, file_size)
    - [ ] T0.4: Create download.ts API module:
      ```typescript
      export function getVideoDownloadUrl(taskId: number): Promise<DownloadUrlResponse>
      export function getAssetsPackDownloadUrl(taskId: number): Promise<DownloadUrlResponse>
      ```
    - [ ] T0.5: Create useDownload.ts composable:
      - State: `isDownloading`, `downloadProgress`, `error`
      - Actions: `downloadVideo(taskId)`, `downloadAssetsPack(taskId)`
      - Helper: `triggerDownload(url, filename)` - create hidden anchor and click
  - **Complexity**: Low
  - **AC Traceability**: Foundation for AC1, AC2

---

### Feature Implementation Tasks

### AC1: 下载成品视频

- [ ] **T1: 视频下载签名URL服务** `[AC1]`
  - **File Locations**:
    - `backend/task-service/src/main/java/com/shopvideoscout/task/service/impl/DownloadServiceImpl.java` (create)
    - `backend/task-service/src/main/java/com/shopvideoscout/task/util/FilenameUtils.java` (create)
    - `backend/task-service/src/test/java/com/shopvideoscout/task/service/DownloadServiceTest.java` (create)
  - **Subtasks**:
    - [ ] T1.1: Implement DownloadServiceImpl.generateVideoDownloadUrl():
      - Validate task exists and status == 'done'
      - Validate output_oss_key is not empty
      - Query task to get shop_name
      - Generate filename: `{sanitizedShopName}-探店视频-{yyyyMMdd}.mp4`
      - Call OssClient to generate signed URL with 3600s expiry
      - Return DownloadUrlResponse with URL, filename, expires_at, file_size
    - [ ] T1.2: Implement FilenameUtils.sanitize(shopName):
      - Replace special chars (/\:*?"<>|) with hyphen
      - Trim whitespace
      - Max length 50 chars
    - [ ] T1.3: Implement DownloadController.getVideoDownloadUrl():
      - Endpoint: GET /api/v1/tasks/{id}/output/download
      - Call DownloadService.generateVideoDownloadUrl(taskId)
      - Return R<DownloadUrlResponse>
    - [ ] T1.4: Configure OSS signed URL parameters:
      - Method: GET
      - Expiration: 3600 seconds
      - Content-Disposition: `attachment; filename="{filename}"`
  - **Complexity**: Low
  - **Error Handling**:
    | Scenario | Handler |
    |----------|---------|
    | Task not found | Throw BusinessException(TASK_NOT_FOUND) |
    | Task not done | Throw BusinessException(OUTPUT_NOT_READY) |
    | OSS signing failed | Throw BusinessException(OSS_ERROR), log error |

  **Test Specs** (unit):
  | Scenario | Input | Expected | Level |
  |----------|-------|----------|-------|
  | Generate signed URL success | task done, output exists | signed URL returned | unit |
  | Task not found | non-existent taskId | TASK_NOT_FOUND error | unit |
  | Task not done | task status=composing | OUTPUT_NOT_READY error | unit |
  | No output file | output_oss_key=null | OUTPUT_NOT_READY error | unit |
  | Filename sanitization | "海底捞/望京店" | "海底捞-望京店" | unit |
  | Filename special chars | "店名:test*file" | "店名-test-file" | unit |
  | Filename max length | 60-char shop name | truncated to 50 chars | unit |
  | Signed URL expiry | any | expires_at = now + 3600s | unit |

---

### AC2: 下载素材包

- [ ] **T2: 素材包打包服务** `[AC2]`
  - **File Locations**:
    - `backend/task-service/src/main/java/com/shopvideoscout/task/service/AssetPackService.java` (create)
    - `backend/task-service/src/main/java/com/shopvideoscout/task/service/impl/AssetPackServiceImpl.java` (create)
    - `backend/task-service/src/main/java/com/shopvideoscout/task/dto/AssetPackResponse.java` (create)
    - `backend/task-service/src/test/java/com/shopvideoscout/task/service/AssetPackServiceTest.java` (create)
  - **Subtasks**:
    - [ ] T2.1: Create AssetPackService interface:
      ```java
      AssetPackResponse generateAssetPack(Long taskId);
      ```
    - [ ] T2.2: Implement AssetPackServiceImpl.generateAssetPack():
      - Query videos where task_id={id} and is_recommended=true
      - Check for cached ZIP (Redis key: assets-pack:{taskId}:{videosHash})
      - If cached, return cached OSS URL
      - If not cached:
        - Download each video from OSS to temp directory
        - Create ZIP with files named: {sequence}-{category}.mp4
        - Upload ZIP to OSS: assets-pack/{task_id}/pack.zip
        - Cache the OSS key in Redis (TTL 1 hour)
        - Return signed download URL
    - [ ] T2.3: Implement ZIP creation with timeout:
      - Max videos: 10
      - Timeout: 60 seconds
      - Use ZipOutputStream for streaming
    - [ ] T2.4: Add endpoint to DownloadController:
      - GET /api/v1/tasks/{id}/assets-pack
      - Return R<AssetPackResponse>
    - [ ] T2.5: Implement cleanup of temp files after ZIP creation
    - [ ] T2.6: Handle partial failures (skip failed videos, continue packing)
  - **Complexity**: Medium
  - **Error Handling**:
    | Scenario | Handler |
    |----------|---------|
    | No recommended videos | Throw BusinessException(NO_ASSETS_TO_PACK) |
    | Download timeout | Throw BusinessException(TIMEOUT), log error |
    | Single video download failed | Log warning, continue with other videos |
    | ZIP creation failed | Throw BusinessException(ASSET_PACK_FAILED) |

  **Test Specs** (unit):
  | Scenario | Input | Expected | Level |
  |----------|-------|----------|-------|
  | Generate assets pack success | 3 recommended videos | ZIP with 3 files | unit |
  | No recommended videos | no is_recommended=true | NO_ASSETS_TO_PACK error | unit |
  | Cache hit | cached ZIP exists | return cached URL, no re-pack | unit |
  | Cache miss | no cache | create ZIP, cache result | unit |
  | ZIP filename format | task with shop_name | "{shopName}-素材包-{date}.zip" | unit |
  | Inner file naming | video category="美食" | "01-美食.mp4" | unit |
  | Max 10 videos | 15 recommended | pack first 10 only | unit |
  | Partial failure | 1 of 3 videos fails download | ZIP with 2 files, log warning | unit |
  | Timeout handling | slow OSS | TIMEOUT error after 60s | integration |

---

### AC1 + AC2: 前端下载组件

- [ ] **T3: 下载按钮UI组件** `[AC1, AC2]`
  - **File Locations**:
    - `frontend/src/components/preview/DownloadButtons.vue` (create)
    - `frontend/src/views/VideoPreviewView.vue` (modify - add DownloadButtons)
    - `frontend/src/types/download.ts` (create)
    - `frontend/src/components/preview/__tests__/DownloadButtons.spec.ts` (create)
  - **Subtasks**:
    - [ ] T3.1: Create download.ts types:
      ```typescript
      export interface DownloadUrlResponse {
        download_url: string;
        filename: string;
        expires_at: string;
        file_size: number;
      }
      export interface AssetPackResponse {
        download_url: string;
        filename: string;
        file_count: number;
        total_size: number;
        expires_at: string;
      }
      ```
    - [ ] T3.2: Create DownloadButtons.vue component:
      - Props: `taskId: number`, `hasRecommendedShots: boolean`
      - Display: "下载视频" button (primary), "下载素材包" button (secondary)
      - Use useDownload composable for state management
      - Disable "下载素材包" if hasRecommendedShots=false
    - [ ] T3.3: Implement download video flow:
      - Click → show loading → fetch signed URL → trigger browser download → success toast
      - Error → error toast with retry option
    - [ ] T3.4: Implement download assets pack flow:
      - Click → show "打包中..." → fetch/create ZIP → trigger download → success toast
      - Handle longer wait time with loading indicator
    - [ ] T3.5: Integrate DownloadButtons into VideoPreviewView.vue:
      - Position below video player
      - Pass taskId and hasRecommendedShots props
    - [ ] T3.6: Handle expired URL (401) by auto-refreshing
  - **Complexity**: Medium
  - **Error Handling**:
    | Scenario | Handler |
    |----------|---------|
    | API error | Show toast with error message |
    | URL expired (during download) | Auto-refresh URL, retry download |
    | No recommended shots | Disable button, show tooltip |

  **Test Specs** (unit):
  | Scenario | Input | Expected | Level |
  |----------|-------|----------|-------|
  | Render both buttons | default props | 2 buttons visible | unit |
  | Download video button click | click | API called, loading shown | unit |
  | Download complete | API success | toast "下载完成" | unit |
  | Download error | API error | toast with error, button enabled | unit |
  | Assets button disabled | hasRecommendedShots=false | button disabled + tooltip | unit |
  | Assets pack loading | click assets | shows "打包中..." | unit |
  | Auto-refresh on 401 | URL expired | refetch URL, retry download | unit |

---

### Integration & Verification Tasks

- [ ] **T4: 集成测试与验证** `[ALL ACs]`
  - **File Locations**:
    - `backend/task-service/src/test/java/com/shopvideoscout/task/controller/DownloadControllerTest.java` (create)
    - `backend/task-service/src/test/java/com/shopvideoscout/task/integration/DownloadIntegrationTest.java` (create)
  - **Subtasks**:
    - [ ] T4.1: Write DownloadController unit tests:
      - GET /tasks/{id}/output/download returns signed URL
      - GET /tasks/{id}/assets-pack returns pack URL
      - Auth required for both endpoints
      - Only task owner can download
    - [ ] T4.2: Write integration tests:
      - Full flow: create task → compose → download video
      - Assets pack flow with caching
      - Error scenarios
    - [ ] T4.3: E2E test (manual): complete download flow in browser
    - [ ] T4.4: Verify OSS signed URL works with Content-Disposition
  - **Complexity**: Low

- [ ] **T5: Final Verification** `[ALL ACs]`
  - [ ] T5.1: All tests passing (unit + integration)
  - [ ] T5.2: No lint errors (frontend + backend)
  - [ ] T5.3: Dev Log complete with implementation summary
  - [ ] T5.4: Status → Review

### AC Coverage Matrix

| Task | AC1 | AC2 |
|------|:---:|:---:|
| T0: Infrastructure | ✓ | ✓ |
| T1: Video Download Service | ✓ |   |
| T2: Assets Pack Service |   | ✓ |
| T3: Download UI Components | ✓ | ✓ |
| T4: Integration Tests | ✓ | ✓ |
| T5: Final | ✓ | ✓ |

## Dev Notes

### Technical Constraints

- **OSS签名URL**: 使用阿里云OSS SDK生成presigned URL，设置Content-Disposition强制下载 [→ 8-infrastructure.md#83-oss]
- **文件名编码**: URL中的中文文件名需要使用RFC 5987编码 (`filename*=UTF-8''...`) [→ HTTP/1.1 Content-Disposition]
- **ZIP流式处理**: 使用ZipOutputStream直接写入临时文件，避免内存溢出 [→ Java ZIP best practices]
- **下载超时**: 前端使用fetch API时无法获取下载进度，仅显示loading状态 [→ Browser限制]
- **Redis缓存**: 素材包OSS key缓存在Redis，TTL 1小时，避免重复打包 [→ 5-data-architecture.md#52-redis]
- **任务隔离**: 通过@TaskOwnership注解验证当前用户是任务所有者 [→ Story 1.4 security]

### Accumulated Context (From Previous Stories)

| Resource Type | Name | Source Story | Action | Key Info |
|--------------|------|--------------|--------|----------|
| Table | tasks | 1.1 (ext 4.3) | READ | shop_name (用于文件名), status, output_oss_key, output_file_size |
| Table | videos | 1.1 | READ | oss_key, category, is_recommended, task_id |
| Endpoint | GET /tasks/{id} | 1.4 | REUSE | 获取任务详情 |
| Endpoint | GET /tasks/{id}/output | 4.3 | REUSE | 获取输出信息（可选，可直接查tasks表） |
| Model | R\<T\> | 1.1 | REUSE | 统一响应包装器 |
| Model | ResultCode | 1.1 (ext 4.3) | EXTEND | 新增NO_ASSETS_TO_PACK(1026), ASSET_PACK_FAILED(1027), OSS_ERROR(1028) |
| Model | OssConfig | 4.1 | REUSE | OSS配置和签名工具 |
| Model | OssClient | 2.1 | REUSE | OSS操作客户端 |
| Annotation | @TaskOwnership | 1.4 | REUSE | 任务所有者验证 |
| Store | usePreviewStore | 5.1 | EXTEND | 添加下载相关状态（可选） |
| Component | VideoPreviewView | 5.1 | MODIFY | 集成DownloadButtons |
| Type | TaskOutput | 5.1 | REUSE | 输出信息类型定义 |

### Architecture Flow (CRITICAL)

**视频下载流程：**

```
Client (Browser)
    │
    ├── Click "下载视频"
    │     ↓
    │── GET /api/v1/tasks/{id}/output/download
    │     │
    │     ├── Auth: JWT验证 + @TaskOwnership
    │     ├── Validate: task.status == 'done', output_oss_key exists
    │     ├── Generate: signed URL (3600s expiry)
    │     │     └── OssClient.generatePresignedUrl(
    │     │           bucket, key, 3600,
    │     │           ResponseHeaderOverrides{
    │     │             ContentDisposition: "attachment; filename=..."
    │     │           })
    │     └── Return: { download_url, filename, expires_at, file_size }
    │     ↓
    ├── Create hidden <a href={url} download={filename}>
    │     └── a.click() → Browser initiates download
    │
    └── Download complete → Toast notification
```

**素材包下载流程：**

```
Client (Browser)
    │
    ├── Click "下载素材包"
    │     ↓
    │── GET /api/v1/tasks/{id}/assets-pack
    │     │
    │     ├── Auth: JWT验证 + @TaskOwnership
    │     ├── Query: videos WHERE task_id={id} AND is_recommended=true
    │     │     └── LIMIT 10 ORDER BY sequence
    │     │
    │     ├── Check Redis cache: assets-pack:{taskId}:{videosHash}
    │     │     ├── HIT → Return cached OSS URL (generate new signed URL)
    │     │     └── MISS ↓
    │     │
    │     ├── Create ZIP (Async/timeout 60s):
    │     │     ├── For each video:
    │     │     │     ├── Download from OSS to temp
    │     │     │     └── Add to ZIP: {seq}-{category}.mp4
    │     │     ├── Upload ZIP to OSS: assets-pack/{task_id}/pack.zip
    │     │     └── Cache OSS key in Redis (TTL 1h)
    │     │
    │     └── Return: { download_url, filename, file_count, total_size, expires_at }
    │     ↓
    ├── Trigger browser download (same as video)
    │
    └── Download complete → Toast notification
```

### Data Models

**DownloadUrlResponse** (成品视频下载响应):
```json
{
  "code": 0,
  "message": "success",
  "data": {
    "download_url": "https://oss.../signed-url?...",
    "filename": "海底捞望京店-探店视频-20260203.mp4",
    "expires_at": "2026-02-03T16:00:00Z",
    "file_size": 47841280
  }
}
```

**AssetPackResponse** (素材包下载响应):
```json
{
  "code": 0,
  "message": "success",
  "data": {
    "download_url": "https://oss.../signed-url?...",
    "filename": "海底捞望京店-素材包-20260203.zip",
    "file_count": 5,
    "total_size": 125829120,
    "expires_at": "2026-02-03T16:00:00Z"
  }
}
```

**Error Response** (任务未完成):
```json
{
  "code": 1025,
  "message": "视频尚未完成合成",
  "details": {
    "task_id": 12345,
    "current_status": "composing"
  }
}
```

**Error Response** (无推荐镜头):
```json
{
  "code": 1026,
  "message": "无推荐镜头可下载",
  "details": {
    "task_id": 12345
  }
}
```

### File Locations

```
backend/
  task-service/
    src/main/java/com/shopvideoscout/task/
      controller/
        DownloadController.java           # 新增: GET /download, GET /assets-pack
      service/
        DownloadService.java              # 新增: 接口
        AssetPackService.java             # 新增: 接口
        impl/
          DownloadServiceImpl.java        # 新增: 签名URL生成
          AssetPackServiceImpl.java       # 新增: ZIP打包服务
      dto/
        DownloadUrlResponse.java          # 新增
        AssetPackResponse.java            # 新增
      util/
        FilenameUtils.java                # 新增: 文件名清理工具
    src/test/java/com/shopvideoscout/task/
      controller/
        DownloadControllerTest.java       # 新增
      service/
        DownloadServiceTest.java          # 新增
        AssetPackServiceTest.java         # 新增
      integration/
        DownloadIntegrationTest.java      # 新增

  common/common-core/
    src/main/java/com/shopvideoscout/common/
      result/
        ResultCode.java                   # 扩展: +NO_ASSETS_TO_PACK, +ASSET_PACK_FAILED, +OSS_ERROR

frontend/
  src/
    api/
      download.ts                         # 新增: API调用
    composables/
      useDownload.ts                      # 新增: 下载状态管理
    types/
      download.ts                         # 新增: 类型定义
    components/
      preview/
        DownloadButtons.vue               # 新增: 下载按钮组件
        __tests__/
          DownloadButtons.spec.ts         # 新增
    views/
      VideoPreviewView.vue                # 修改: 集成DownloadButtons
```

### Deliverable Bindings

> **SM**: Binding for each new code unit. Dev Gate will verify these.
> **Dev**: Ensure all bindings are implemented before self-review.

```yaml
deliverable_bindings:
  # Backend new classes
  - deliverable: "backend/task-service/.../controller/DownloadController.java"
    consumer: "backend/gateway/src/main/resources/application.yml"
    binding_type: route_mount
    verify: "Path=/api/v1/tasks/**"

  - deliverable: "backend/task-service/.../service/impl/DownloadServiceImpl.java"
    consumer: "backend/task-service/.../controller/DownloadController.java"
    binding_type: di_inject
    verify: "DownloadService"

  - deliverable: "backend/task-service/.../service/impl/AssetPackServiceImpl.java"
    consumer: "backend/task-service/.../controller/DownloadController.java"
    binding_type: di_inject
    verify: "AssetPackService"

  - deliverable: "backend/task-service/.../dto/DownloadUrlResponse.java"
    consumer: "backend/task-service/.../controller/DownloadController.java"
    binding_type: import_usage
    verify: "DownloadUrlResponse"

  - deliverable: "backend/task-service/.../dto/AssetPackResponse.java"
    consumer: "backend/task-service/.../controller/DownloadController.java"
    binding_type: import_usage
    verify: "AssetPackResponse"

  - deliverable: "backend/task-service/.../util/FilenameUtils.java"
    consumer: "backend/task-service/.../service/impl/DownloadServiceImpl.java"
    binding_type: import_usage
    verify: "FilenameUtils"

  # Frontend new components
  - deliverable: "frontend/src/components/preview/DownloadButtons.vue"
    consumer: "frontend/src/views/VideoPreviewView.vue"
    binding_type: import_usage
    verify: "DownloadButtons"

  - deliverable: "frontend/src/composables/useDownload.ts"
    consumer: "frontend/src/components/preview/DownloadButtons.vue"
    binding_type: import_usage
    verify: "useDownload"

  - deliverable: "frontend/src/api/download.ts"
    consumer: "frontend/src/composables/useDownload.ts"
    binding_type: import_usage
    verify: "getVideoDownloadUrl"
```

**Binding Status**: ⏳ Pending Dev verification

### Testing Requirements

- **单元测试重点**:
  - OSS签名URL生成参数验证（过期时间、Content-Disposition）
  - 文件名清理逻辑（特殊字符、长度限制）
  - ZIP打包文件命名规则
  - 任务状态验证
  - 缓存命中/未命中逻辑
- **集成测试重点**:
  - 完整下载流程（签名URL有效性）
  - 素材包打包超时处理
  - Redis缓存功能
  - 401重试机制
- **边界测试**:
  - 超长店铺名文件名处理
  - 无推荐镜头时的错误处理
  - 打包超过10个视频时截断
  - 部分视频下载失败时的继续打包

---

## AC Traceability Matrix

> **Dev**: Fill this section BEFORE self-review. Every AC needs evidence.
> **QA**: Verify each entry during AC Coverage Verification.

### AC1: 下载成品视频

```yaml
ac_id: AC1
code_locations:
  - "backend/task-service/src/main/java/com/shopvideoscout/task/service/impl/DownloadServiceImpl.java"
  - "backend/task-service/src/main/java/com/shopvideoscout/task/controller/DownloadController.java"
  - "frontend/src/components/preview/DownloadButtons.vue"
test_locations:
  - "backend/task-service/src/test/java/com/shopvideoscout/task/service/DownloadServiceTest.java"
  - "frontend/src/components/preview/__tests__/DownloadButtons.spec.ts"
verification_type: unit_test
aspects_covered:
  main_scenario: true
  business_rules: true
  data_validation: true
  error_handling: true
notes: "Signed URL generation, filename sanitization, Content-Disposition, 1hr expiry"
```

### AC2: 下载素材包

```yaml
ac_id: AC2
code_locations:
  - "backend/task-service/src/main/java/com/shopvideoscout/task/service/impl/AssetPackServiceImpl.java"
  - "backend/task-service/src/main/java/com/shopvideoscout/task/controller/DownloadController.java"
  - "frontend/src/components/preview/DownloadButtons.vue"
test_locations:
  - "backend/task-service/src/test/java/com/shopvideoscout/task/service/AssetPackServiceTest.java"
  - "frontend/src/components/preview/__tests__/DownloadButtons.spec.ts"
verification_type: unit_test
aspects_covered:
  main_scenario: true
  business_rules: true
  data_validation: true
  error_handling: true
notes: "ZIP creation, caching, timeout handling, partial failure recovery, 10-video limit"
```

---

### Traceability Summary

| AC | Code Location | Test Location | Type | Status |
|----|---------------|---------------|------|--------|
| AC1 | DownloadServiceImpl.java, DownloadController.java, DownloadButtons.vue | DownloadServiceTest.java, DownloadButtons.spec.ts | unit | ✅ |
| AC2 | AssetPackServiceImpl.java, DownloadController.java, DownloadButtons.vue | AssetPackServiceTest.java, DownloadButtons.spec.ts | unit | ✅ |

**Legend**: ✅ Verified | ⏳ Pending | ❌ Missing

---

## Agent Records

### Architect Review

```yaml
review_required: false
review_round: 1
review_status: APPROVED
score: 9.5
issues_critical: 0
issues_major: 0
issues_minor: 3
minor_issues:
  - m1: Error code format inconsistency - mix of HTTP-style (40301/40303) and app codes (1025-1028)
  - m2: Partial failure response code - 500 for continuing operation is confusing
  - m3: Videos hash calculation not specified in BR-2.7
```

### QA Test Design

```yaml
test_design_level: Standard
test_design_status: Complete
assigned_date: 2026-02-03
completed_date: 2026-02-03
test_scenarios: 108
test_document: docs/qa/assessments/5.2-test-design-20260203.md
```

---

## Architect Review Results

**Review Date**: 2026-02-03
**Reviewer**: Aiden (Solution Architect Agent)
**Score**: 9.5/10
**Decision**: Approved
**Test Design Level**: Standard

### Validation Summary

- ✅ YAML metadata block complete
- ✅ T0-T5 tasks with detailed subtasks and file locations
- ✅ Both frontend and backend implementation specified
- ✅ AC traceability matrix pre-defined
- ✅ Deliverable bindings with verification criteria
- ✅ Architecture flow diagrams for both download paths
- ✅ OSS signed URL with Content-Disposition for forced download
- ✅ Redis caching for asset pack with TTL
- ✅ RFC 5987 encoding consideration for Chinese filenames
- ✅ Accumulated context references Stories 1.1, 1.4, 2.1, 4.1, 4.3, 5.1

### Minor Issues (Non-blocking)

| ID | Issue | Recommendation |
|----|-------|----------------|
| m1 | Error code format inconsistency | Standardize: use ResultCode (1025-1028) throughout, remove HTTP-style 40301/40303 |
| m2 | Partial failure response code | Use 200 with warnings array or 207 Multi-Status instead of 500 |
| m3 | Videos hash not specified | Document hash algorithm (e.g., `MD5(sorted video IDs joined by comma)`) |

---

## Change Log

| Date | Agent | Status Transition | Details/Link |
|------|-------|-------------------|--------------|
| 2026-01-22 | SM | Created → Draft | Story created from PRD Epic 5 |
| 2026-02-03 | SM | Draft → AwaitingArchReview | Full elaboration with tasks, API specs, test specs |
| 2026-02-03 | Architect | AwaitingArchReview → AwaitingTestDesign | Score 9.5/10, 0 critical, 0 major, 3 minor. Approved for test design. |
| 2026-02-03 | QA | AwaitingTestDesign → Approved | Test design complete: 108 scenarios (78 unit, 8 integration, 4 E2E, 18 blind spot). Doc: 5.2-test-design-20260203.md. Ready for Dev. |
| 2026-02-03 | Dev | Approved → Review | Frontend implementation complete: 6 files created, 1 modified, 47 new tests (557 total passing). Dev log: 5.2-dev-log.md. HANDOFF TO qa |
| 2026-02-03 | QA | Review (BLOCKED) | Gate BLOCKED: Frontend complete (100% tests), but backend NOT IMPLEMENTED. AC coverage 50%. Missing: DownloadController, AssetPackService. HANDOFF BACK TO dev |
| 2026-02-04 | Dev | Review → Review | Backend implementation complete: DownloadController, DownloadService, AssetPackService, FilenameUtils, DTOs. 13 backend files created, 39 backend tests. Dev log updated. HANDOFF TO qa |
| 2026-02-04 | Dev | Review | Backend files committed to git (commit 3fd6633). 13 files, 1405 insertions. Ready for QA. HANDOFF TO qa |
