...
**As a** 用户,
**I want** 查看和管理我的历史任务,
**so that** 可以回顾之前生成的视频并清理不需要的任务

### 🔌 APIs Provided by This Story

- `DELETE /api/v1/tasks/{id}` (task-service - 删除任务及相关OSS文件)

### 🔌 APIs Consumed by This Story

- `GET /api/v1/tasks` (from Story 1.4 - 获取用户任务列表，支持分页)
- `GET /api/v1/tasks/{id}` (from Story 1.4 - 获取任务详情)
...
**Story Deliverables**:
- 历史任务列表页面 (TaskHistoryView)
- 任务卡片组件 (TaskHistoryCard)
- 无限滚动加载 (useInfiniteScroll composable)
- 删除确认弹窗 (DeleteConfirmModal)
- 删除任务API (TaskController.deleteTask)
- OSS文件清理服务 (OssCleanupService)
...
|----|------|
| BR-1.1 | 每页显示10条任务 |
| BR-1.2 | 显示任务状态图标（completed=绿色勾, composing=蓝色进度, failed=红色叉） |
| BR-1.3 | 显示店铺类型标签（餐饮/美容/娱乐/其他） |
| BR-1.4 | 显示创建时间，使用相对时间格式（如"2小时前"、"3天前"） |
| BR-1.5 | 任务状态为 completed 时显示封面缩略图 |
| BR-1.6 | 空列表显示引导创建任务的提示 |
...
|---------|----------|
| 页面加载 | 加载第一页任务 |
| 滚动到底部 | 自动加载下一页（有更多数据时） |
| 点击任务卡片 | 根据状态跳转：completed→预览页，其他→对应状态页 |
| 下拉刷新 | 重新加载第一页 |
...
|----|------|
| BR-2.1 | 删除后无法恢复（硬删除） |
| BR-2.2 | 同时删除OSS上的所有相关文件（视频、帧、音频、输出视频、素材包） |
| BR-2.3 | 数据库CASCADE删除关联表（videos, scripts, publish_assists） |
| BR-2.4 | 只能删除自己的任务（user_id校验） |
| BR-2.5 | 正在进行中的任务（analyzing/composing）不能删除 |

**Data Validation**
| Field | Type | Required | Rules | Error Message |
|-------|------|----------|-------|---------------|
| task_id | Long | Y | must exist | 任务不存在 |
| task.user_id | Long | Y | must match current user | 无权限删除此任务 |
| task.status | string | Y | not in ['analyzing', 'composing'] | 任务正在处理中，无法删除 |

**Error Handling**
| Scenario | Code | Message | Action |
|----------|------|---------|--------|
| 任务不存在 | 404 | 任务不存在 | 关闭弹窗，从列表移除 |
| 无权限 | 403 | 无权限删除此任务 | 显示错误Toast |
| 任务进行中 | 409 | 任务正在处理中，无法删除 | 显示错误Toast |
| 删除失败 | 500 | 删除失败，请重试 | 显示重试按钮 |
| OSS删除部分失败 | - | - | 记录日志，不影响任务删除 |
...
- [x] **T1: 任务列表页面实现** `[AC1]`
  ...
  - **Subtasks**:
    ...
    - [x] T1.5: Implement relative time formatting:
      ...
      - Format: "刚刚", "5分钟前", "2小时前", "3天前", "2026-01-15"
    ...
    - [x] T1.7: Implement navigation based on task status:
      - completed → /task/:id/preview
      - script_ready/script_edited → /task/:id/script
      - composing → /task/:id/compose-progress
      - Other → /task/:id (detail page)
  - **Complexity**: Medium
  - **Error Handling**:
    | Scenario | Handler |
    |----------|---------|
    | Load error | Show error state with retry button |
    | No tasks | Show EmptyState with "创建任务" CTA |
  ...
  **Test Specs** (unit):
  ...
  | Relative time | created_at 2h ago | "2小时前" | unit |
...
- [x] **T4: 导航集成** `[ALL ACs]`
  ...
  - **Subtasks**:
    - [x] T4.1: Add "历史任务" entry to bottom navigation or sidebar
...
└── DeleteConfirmModal.vue (shared modal)
```
...
|----|---------------|---------------|------|--------|
| AC1 | TaskHistoryView.vue, TaskHistoryCard.vue, useTaskHistory.ts | TaskHistoryCard.test.ts, useTaskHistory.spec.ts | unit | ✅ |
| AC2 | DeleteConfirmModal.vue, TaskController.java, TaskService.java, OssCleanupService.java | DeleteConfirmModal.test.ts, TaskServiceDeleteTest.java | unit | ✅ |
...
|------------|-----------|--------|
| CASCADE DELETE | §5.1 Foreign Keys | ✅ videos, scripts, publish_assists |
| OSS Storage | §5.3 OSS Structure | ✅ All paths correct |
| Frontend structure | §12.1 | ✅ views/, components/, composables/ |
| Backend structure | task-service | ✅ controller/, service/ |
| API design | §6 | ✅ RESTful, pagination |
| Security | ownership validation | ✅ user_id check |
...
| 2026-02-06 | SM | Draft → AwaitingArchReview | Full elaboration: TaskHistoryView, TaskHistoryCard, DeleteConfirmModal, useInfiniteScroll, useTaskHistory composables, DELETE API with OSS cleanup, pagination support, status-based navigation. |
| 2026-02-06 16:30 | Architect | AwaitingArchReview → Approved | Score: 10.0/10, No issues. Simple tier - ready for Dev implementation |
| 2026-02-06 | Dev | Approved → Review | Implementation complete: TaskHistoryView, TaskHistoryCard, DeleteConfirmModal, EmptyState, useInfiniteScroll, useTaskHistory; backend DELETE API with OssCleanupService; pagination support; 53 unit tests. See dev log 5.5-dev-log.md |
